...
function TagList(props) {
    ...
    const [cookies2, setCookie2] = useCookies(['tagnameSelect']);
    const targetRef = useRef([]);

    const tagClickEvent = (e, tagname) => {
        props.tagClick(tagname);
        setCookie('tagSelect', e);
        setCookie2('tagnameSelect', tagname);

        targetRef.current.map(tg => { tg.style.backgroundColor = 'rgba(100, 100, 170, 0.3)'; tg.style.color = 'rgb(210, 210, 210)'; return null; });
    ...
    useEffect(() => {
        if (cookies.tagSelect >= 0) {
            if (targetRef.current[cookies.tagSelect]) {
                const name = cookies2.tagnameSelect;
                props.tagClick(name);
